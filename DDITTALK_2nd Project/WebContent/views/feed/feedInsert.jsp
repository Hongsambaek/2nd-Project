...
<%
	MemberVO mv20331 = (MemberVO) session.getAttribute("LOGIN_USER");
%>

<div class="container">
	<form class="form-feed-insert" method="post" onsubmit="valid(event)"
		enctype="multipart/form-data"
		action="<%=request.getContextPath()%>/feedinsert.do">
		<div class="mb-3">
			<div id="dispp1" class="dispp border border-danger">이미지를 선택하세요</div>
			<input id="inputBtn1" type="file" class="form-control mt-2"
				value="파일 선택" name="fileId">
		</div>
		<div class="mb-3">
			<textarea placeholder="내용을 입력하세요(검색어는 해시태그(#)로 입력하세요)"
				class="form-control" id="feedContent1" name="feedContent"></textarea>
		</div>
		<input id="insertSpan" name="memEmail" value="<%=mv20331.getMemEmail()%>"
